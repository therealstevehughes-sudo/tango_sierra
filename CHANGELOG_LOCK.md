...
Deferred items: Everything already deferred in Sprint 001 remains deferred (TaskTemplate/TaskInstance split, sync-state fields, site/area/user linkage, real photo capture, real auth/roles). *(Superseded 2026-09-13: real photo capture built in Sprint 032 P0.)*
...
Deferred items: Everything already deferred in Sprint 003 remains deferred (staff-management screen, director role tier, sync fields, TaskTemplate/TaskInstance split, real photo capture), plus a way for managers to access the task carousel, plus removing the now-fully-dead `_mockStaff` code in TaskController. *(Superseded 2026-09-13: real photo capture built in Sprint 032 P0.)*
...
Deferred items: The self-service unit-preference toggle UI (Sprint 016), TaskInstance / due-date-aware queue filtering, role-wide assignment, real photo capture, cross-unit conversion beyond Celsius/Fahrenheit. *(Superseded 2026-09-13: real photo capture built in Sprint 032 P0.)*
...
## Sprint 032 (P0) — Photo evidence: real capture, on-disk persistence, PDF embed
Date: 2026-09-13
Objective: Sprint 032 P0, build order item 1 per PHOTO_EVIDENCE_PLAN.md. Make the anti-fraud promise real: TaskSubmission.photoPath is now actually written by a real camera capture, not a boolean toggle. Camera-first capture (explicit gallery fallback), JPEG persisted into <documents>/evidence/, and the EHO export's full detailed log embeds the real bytes.
Files changed:
- pubspec.yaml / pubspec.lock — added `image_picker: ^1.1.2` (the ONLY new dependency this sprint; an earlier draft had it listed twice — deduped).
- lib/core/services/evidence_store.dart (new) — `EvidenceStore`: `pickAndPersistPhoto()` (camera → gallery fallback; null on cancel, never throws), copies JPEG into `<app documents>/evidence/`, returns a stable path; `readPhotoBytes` is best-effort (null on missing/unreadable). Provider `evidenceStoreProvider`. Files on disk, never BLOBs in Drift.
- lib/features/tasks/task_screen.dart — "Add Photo" now runs a real capture via `_capturePhoto()`; button disables once taken; `photoPath` state flows through `logTaskSubmission` (previously never set); reset between tasks.
- lib/features/tasks/task_controller.dart — `logTaskSubmission` accepts + forwards `photoPath` (and `logRemainingAsNotCompleted` stays photo-free by design).
- lib/features/export/eho_export_service.dart — preloads photo bytes in `generate()` (async I/O kept out of the synchronous layout pass); full detailed log embeds the actual image (96px, BoxFit.cover) with the task title as caption via `_photoOrPlainCell`; corrupt/missing bytes degrade to the `[Photo attached]` marker — never breaks the export. Limitations notice updated to state photos are now embedded when the full log is included.
- ios/Runner/Info.plist — `NSCameraUsageDescription` + `NSPhotoLibraryUsageDescription` keys (required by image_picker on iOS).
Files unchanged: models, repositories (drift/supabase), auth, tiers, drawer, export dialog UI, themes, guided_task_header — no schema change (photoPath column already existed in schemaVersion 1).
Architecture impact: None to ARCHITECTURE_LOCK's repository rule — UI still reads/writes only through repositories; the EvidenceStore sits under core/services as a platform capability service, same neighbourhood as backup/export services. Storage stays outside Drift (path reference only), preserving the offline-first + sync-ready story: when a real backend arrives, the path reference swaps to a URL with no model change.
UI impact: The Add Photo button now launches the real camera/gallery flow; a clear text marker replaces the old instant "Photo Added". No other layout/wording changed.
Risks: Real photo capture depends on the device camera — image_picker's runtime permission flow applies on Android; iOS keys added. A photo whose stored file is later deleted degrades to the text marker on export (never a broken PDF).
Verified: `flutter analyze` clean; all 14/14 tests passing; committed + pushed (2696b12).
