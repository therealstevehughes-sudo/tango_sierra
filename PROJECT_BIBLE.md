...
- **Venue Manager**: accountable for one venue — sets up the venue, assigns tasks, gets that site's escalations. Venue Manager and above can trigger this venue's inspection/EHO data export (Sprint 031) — reconsidered from an earlier executive-only restriction: an EHO inspection is unannounced and happens at the venue, so requiring the Director specifically would defeat the one-tap-in-the-moment point of the feature.
- **Regional**: oversees a cluster of venues, gets escalations a Venue Manager didn't close.
- **Executive/Director**: company-wide — sets policy and branding, sees everything, owns the audit relationship.
...
- inspection/EHO data export, venue manager tier and above (Sprint 031 — see the Role Tier Model note above)
