...
**Still not done**: which events trigger a push is settled (accident/incident raised, an issue escalated to you, damaged-stock supply problems — everything else batches into an end-of-shift summary instead, per the user's own call), but the actual server-side send logic isn't built yet — needs a Firebase service account key (a real secret, generated in the Firebase console under Project Settings → Service Accounts, stored as a Supabase Edge Function secret, never committed to git) and a new `send-push` Edge Function using FCM's HTTP v1 API, called from `SupabaseIssueRepository` at the trigger points above.
